      ...
      >
        ...
        >
          ...
          <label
            ...
            :class="{
              'fixed inset-0 w-screen h-screen bg-black z-50 flex items-center justify-center rounded-none': isExpanded
            }"
          >
            <img
          ...
          >
            ...
            <p
              class="text-[19px] text-[#4e4a67] mb-6 leading-[1.5em] text-justify"
            >
...
import a from '@/assets/img/project/menaradanareksa.jpg'
import b from '@/assets/img/project/tamanmelatidepok.jpg'
import c from '@/assets/img/project/pelindo.jpg'
...
const fetchCities = async () => {
  try {
    ...
    console.warn('Gagal memuat dari API, menggunakan data fallback:', error)
...
const onTouchEnd = (e) => {
  ...
  if (Math.abs(diff) > 50 && cities.value.length > 0 && !isExpanded.value) {
    if (diff > 0) {
      ...
      // Prev
      selected.value =
        (selected.value - 1 + cities.value.length) % cities.value.length
    }
  }
}
</script>
