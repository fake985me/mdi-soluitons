<template>
  <div id="hero" class="bg-black text-white py-10 relative overflow-hidden">
    <div v-for="(slide, index) in slides" :key="index" v-show="index === currentIndex"
      class="absolute inset-0 transition-opacity duration-1000" :style="{ backgroundImage: `url(${slide.bg})` }">
      ...
      <div class="relative z-10 h-full flex flex-col justify-center items-center text-center px-4 m-4">
        <div v-if="slide.imgLeft || slide.midLeft || slide.midRight || slide.imgRight"
          class="flex flex-col md:flex-row items-center justify-center gap-6 w-full max-w-6xl mx-auto">
          <img v-if="slide.imgLeft" :src="slide.imgLeft" class="w-25 max-w-[220px] h-auto animate-fadeInLeft" />
          <img v-if="slide.midLeft" :src="slide.midLeft" class="w-70 max-w-[140px] h-auto animate-fadeInLeft" />
          <img v-if="slide.midRight" :src="slide.midRight" class="w-80 max-w-[160px] h-auto animate-fadeInRight" />
          <img v-if="slide.imgRight" :src="slide.imgRight" class="w-90 max-w-[180px] h-auto animate-fadeInRight" />
        </div>
    ...
    <div class="absolute bottom-6 flex gap-2 justify-center w-full z-20">
      <button v-for="(slide, index) in slides" :key="index"
        ...
        :class="{ 'bg-blue-500 scale-125': index === currentIndex }" @click="goToSlide(index)">
      </button>
    </div>
  </div>
</template>
...
const slides = [
  {
    bg: new URL('@/assets/img/hero-carousel/2.jpg', import.meta.url).href,
    centerImg: new URL('@/assets/img/hero-carousel/dasan.png', import.meta.url).href,
    centerImgWidth: 500,
    centerImgHeight: 250,
    text: 'provide your network connectivity solutions',
  },
  {
    bg: new URL('@/assets/img/hero-carousel/1.jpg', import.meta.url).href,
    centerImg: new URL('@/assets/img/hero-carousel/mdi.png', import.meta.url).href,
    centerImgWidth: 500,
    centerImgHeight: 250,
    text: 'Trusted distributor and Experienced technical support for DASAN equipment in Indonesia',
  },
  {
    bg: new URL('@/assets/img/hero-carousel/3.jpg', import.meta.url).href,
    // centerImg: new URL('@/assets/img/hero-carousel/hero 3.png', import.meta.url).href,
    imgLeft: new URL('@/assets/img/hero-carousel/hero_1.png', import.meta.url).href,
    midLeft: new URL('@/assets/img/hero-carousel/hero_2.png', import.meta.url).href,
    midRight: new URL('@/assets/img/hero-carousel/hero_3.png', import.meta.url).href,
    imgRight: new URL('@/assets/img/hero-carousel/hero_4.png', import.meta.url).href,
